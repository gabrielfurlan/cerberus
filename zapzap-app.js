...
function loadModules() {
  function getStore(modules) {
      ...
      },
      {
        ...
        conditions: module =>
          ...
          module.prototype.constructor
            .toString()
            .indexOf('binaryProtocol deprecated version') >= 0
            ? module
            : null,
      },
      {
    ...
    // IF WE FOUND THE STORE, SUCCESS, OTHERWISE, KEEP TRYING
...
function onLoad() {
  ...
  window.WAPI.getNewMessageId = function(chatId) {
    ...
    newMsgId._serialized = `${newMsgId.fromMe}_${newMsgId.remote}_${
      newMsgId.id
    }`;
  ...
  // Rules
  // 1. Começa a bloquear contas a partir das mensagens não lidas
  // 2.

  console.log('Loaded ZapZap');

  /*
  setInterval(function() {
    var fromBrazil = function(chat) {
      var chatId = chat.id.user;
      return chatId.match(/^55/);
    };

    var chats = window.WAPI.getUnreadMessages();
...
function reactMain() {
  ...
  ReactDOM.render(<App />, document.getElementById('zapzap-app'));
}
